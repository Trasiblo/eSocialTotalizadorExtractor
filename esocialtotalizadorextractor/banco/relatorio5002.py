...
class relatorio5002:
    ...
    def get_valor(self):
        ...
        where
        ex.infoExclusao_tpEvento = 'S-1210'),

        validos as( 
        select t.* from s5002 t
        left join ex on t.nrRecArqBase = ex.infoExclusao_nrRecEvt and ex.infoExclusao_perApur = t.perApur and ex.infoExclusao_cpfTrab = t.cpfBenef
        where ex.id is null and t.vlrRendTrib is not null)

        select 
        s.perApur, 
        count(s.perApur),
        sum(s.vlrRendTrib),
        sum(s.vlrPrevOficial),
        sum(s.vlrCRMen)
        from validos s 
        group by s.perApur
        """
